 ...
 * @Description: fanout 模型消费者
 * @Author: wangzq
 * @Date: 2021/5/16 下午2:21
 */
@Component
public class FanoutConsumer {
    ...
    public void receiveMessage(Message message, Channel channel) {
        ...
        try {
            ...
            System.out.println("FanoutConsumer receive message info:" + JSONObject.toJSON(amqpMessage));
            //处理具体的业务逻辑，处理完成后，记得把返回标志位设置成true ，否则消息被重新放到消息队列或者拒绝丢弃掉

            success = true;
        } catch (Exception e) {
            log.error("fanout模型消费rabbitmq时，解析数据信息异常：e={} | message={}", e.getMessage(), message);
        }
...
//                channel.basicReject(message.getMessageProperties().getDeliveryTag(), false);
                ...
                log.error("fanout模式下消费端拒绝消息异常：e={} | message={}", e.getMessage(), message);
            }
        }
        try {
            /**
            ...
            log.error("fanout模式下消费端消息处理完成，消息确认时异常：e={}| messageId={}", e.getMessage(), amqpMessage.getMessageId());
